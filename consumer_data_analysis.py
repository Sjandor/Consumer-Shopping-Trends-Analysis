...
# Mean and median of monthly spend
def spend(spend):
...
def loyalty_by_gender(data):
    grouped_loyalty = data.groupby('gender')['brand_loyalty_score'].mean()
    most_loyal = grouped_loyalty.idxmax()
    return grouped_loyalty, most_loyal

grouped_loyalty, most_loyal = loyalty_by_gender(consumer_data)
print("\nWhat gender is most loyal to brands on average?")
for gender, score in grouped_loyalty.items():
    print(f"  {gender}: {score:.2f}")
print(f"Most loyal group on average: {most_loyal}\n")

# How does delivery fee sensitivity affect spending habits?
def deliv_fee_vs_spending(data):
    grouped = data.groupby('delivery_fee_sensitivity')['avg_online_spend'].mean()
    global_avg       = data['avg_online_spend'].mean()
    avg_low          = grouped[grouped.index <= 3].mean()
    avg_mid          = grouped[(grouped.index >= 4) & (grouped.index <= 6)].mean()
    avg_high         = grouped[grouped.index >= 7].mean()
    return global_avg, avg_low, avg_mid, avg_high

global_avg, avg_low, avg_mid, avg_high = deliv_fee_vs_spending(consumer_data)
print(f"\nGlobal average online spend:          {global_avg:,.2f}")
print(f"Avg spend - Low sensitivity (1-3):    {avg_low:,.2f}")
print(f"Avg spend - Mid sensitivity (4-6):    {avg_mid:,.2f}")
print(f"Avg spend - High sensitivity (7-10):  {avg_high:,.2f}\n")
...
# How does city tier affect income?
def city_tier_income(data):
    grouped_tier = data.groupby('city_tier')['monthly_income'].mean()
    return grouped_tier

grouped_city_income = city_tier_income(consumer_data)
for tier in ["Tier 1", "Tier 2", "Tier 3"]:
    print(f"{tier} average income:")
    print(f"  Monthly income: ${grouped_city_income[tier]:,.2f}")

# What age group earns the most?
def income_by_age(data):
    grouped_age  = data.groupby('age')['monthly_income'].mean()
    average_income = grouped_age.mean()
    adult_teens  = grouped_age[grouped_age.index < 20].mean()
    young_adults = grouped_age[(grouped_age.index >= 20) & (grouped_age.index <= 25)].mean()
    adults       = grouped_age[(grouped_age.index >= 26) & (grouped_age.index <= 45)].mean()
    middle_aged  = grouped_age[(grouped_age.index >= 46) & (grouped_age.index <= 64)].mean()
    elderly      = grouped_age[(grouped_age.index >= 65) & (grouped_age.index <= 120)].mean()
...
axes[0, 1].set_ylim(grouped_loyalty.min() * 0.9, grouped_loyalty.max() * 1.1)
